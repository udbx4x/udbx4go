...
When a Viewer preview request omits its viewport, the backend uses `ListContext` to create a private, ID-ordered, bounded initial sample. That response uses `bounded_sample` with `QueriedBounds` and `degradedReason` unset; the declared extent may still guide automatic map positioning. `Sampled` and `SampleReason` are derived from the bounded read's actual `HasMore` state and vertex-budget truncation, never from `SmObjectCount`. When a viewport is present, the Viewer calls `QuerySpatial`. Normal Text/CAD queries succeed with `rtree` or `envelope_cache`; only `envelope_cache_budget_exceeded` or `spatial_index_unavailable` may trigger a private bounded fallback, which carries the corresponding `degradedReason`. `bounded_sample` and the Viewer DTO's `degradedReason` are not SDK `SpatialQueryResult` fields or successful SDK strategies.
