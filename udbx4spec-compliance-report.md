...
- Viewer 私有 `bounded_sample` 只用于 `envelope_cache_budget_exceeded` 或 `spatial_index_unavailable` 的非空间有界预览，不属于 SDK 成功结果；正常 Text/CAD 通过公共视口查询返回 `envelope_cache`，Viewer DTO 仅在降级时保留 `degradedReason`
