...
[[ "$peak_rss" == 110 && "$rss_start" == 110 && "$rss_end" == 110 ]]
record_rss_sample 120 2
[[ "$peak_rss" == 120 && "$rss_start" == 110 && "$rss_end" == 120 ]]
record_rss_sample 130 3
[[ "$peak_rss" == 130 && "$rss_start" == 110 && "$rss_end" == 130 ]]

# Repeated or lower samples are valid and must not trip callers using set -e.
record_rss_sample 130 4
record_rss_sample 125 5
[[ "$peak_rss" == 130 && "$rss_start" == 110 && "$rss_end" == 125 ]]

# Runs shorter than one second use the first valid sample as their baseline.
peak_rss=0
rss_first=0
rss_start=0
rss_end=0
