...
当前单数据集 32 MiB、单个 `DataSource` 合计 64 MiB 是经过测量的缓存资源默认策略，按“每数据集约 4 MiB 固定 charge + 每 capacity entry 约 80 bytes”的稳定 RSS 模型计费，不是对象数或 UDBX 格式限制。完整缓存超出该策略时，SDK 返回 `envelope_cache_budget_exceeded` 错误。Text 与 CAD 使用 `SmIndexKey` 过滤包络候选，再从 `SmGeometry` 解码命中的业务对象；该路径要求存在包络列和有效空间元数据。缺少 `SmIndexKey` 或有效 `geometry_columns` 注册的 legacy CAD 不会通过非空间路径伪装成查询成功，而是报告 `spatial_index_unavailable`。请求未提供 viewport 时，Viewer 使用完整的有限 `float64` 坐标域调用 `QuerySpatial`，并保持 `QueriedBounds` 为空；声明 extent 仅作为自动定位提示，不作为完整性边界。采样诊断取决于实际查询的 `HasMore` 和顶点预算截断，不使用 `SmObjectCount` 推断。Viewer 只在 `QuerySpatial` 返回 `envelope_cache_budget_exceeded` 或 `spatial_index_unavailable` 后生成私有的非空间 `bounded_sample` 预览；其 DTO 可以保留 `degradedReason`，但二者都不属于 SDK 成功结果。可运行完整程序、六个原因码和 `ListContext` 取消语义见 [API.zh.md](./API.zh.md#视口空间查询)。
